...
<?php
  ...
  // Taking all 5 values from the form data(input)
	$name = $pass =$uname= $uemail  = $uphone = $uusername = $upassword="";
...
if (!empty($_POST['submit_login'])){
  ...
  if(!empty($name)&&(!empty($pass))){
    $sql="SELECT * FROM user_signup where username='$name' and pass='$pass'";
...
if (!empty($_POST['submit_registration'])) {
  ...
  $upassword = test_input($_POST["upassword"]);
	 ...
	 if(!empty($uname)&&!empty($uemail)&&!empty($uusername)&&!empty($upassword)&&!empty($uphone)){
		$sql = "INSERT INTO user_signup VALUES ('$uname','$uemail','$uphone','$uusername','$upassword')";
